async function mainEvent() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const endDate = new Date(year, month, 0).toLocaleDateString("en-US", {
      month: "2-digit",
      day: "2-digit",
      year: "numeric",
    });
    ...
    ) //An asynchronous data request to my API
  ...
  function createMarker(latitude, longitude, mag, place, time) {
    ...
    marker.bindPopup(
      `<strong>Magnitude:</strong> ${mag}<br><strong>Place:</strong> ${place}<br><strong>Time:</strong> ${new Date(
        time
      )}`
    );
  }
  ...
  function sortTableByMagnitude() {
    ...
    rows.sort((row1, row2) => {
      const magnitude1 = parseFloat(
        row1.querySelector("td:nth-child(2)").textContent
      );
      const magnitude2 = parseFloat(
        row2.querySelector("td:nth-child(2)").textContent
      );
